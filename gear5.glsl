...
vec4 gear( in vec3 p, in float time, in float off )
{
    {
    float an = time*sign(p.y) + off*6.283185/24.0;
...
vec4 map( in vec3 p, float time )
{
    ...
    else if( abs(q.z)>abs(q.y) && abs(q.z)>abs(q.x) ) q = q.xzy*vec3(-1,1,1);
    d1 = gear(q,time, 0.0);
    }
    
    // X
    {
    vec3 q = vec3(rot(p.zy),p.x);
    if(abs(q.x)>abs(q.y)) q=q.zxy;
    vec4 d2 = gear(q,time, 1.0);
    d1 = (d2.x<d1.x)?d2:d1;
    }
    // Y
    {
	vec3 q = vec3(rot(p.xz),p.y);
    if(abs(q.x)>abs(q.y)) q=q.zxy;
    vec4 d2 = gear(q,time, 1.0);
    d1 = (d2.x<d1.x)?d2:d1;
    }
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    for( int n=ZERO; n<AA; n++ )
    {
        ...
        if( tuvw.x>0.0 )
        {
            ...
            vec3 mate = 0.5*texture(iChannel0,tuvw.yz).xyz +
                        0.5*texture(iChannel0,tuvw.yw).xyz;
            mate *= mate;
            mate *= 0.2;
            
            float occ = calcAO( pos+0.001*nor, nor, time );
            float li1 = 0.5+0.5*nor.y;
            col = 15.0*li1*mate*occ;
        }
